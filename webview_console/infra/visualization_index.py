...
class VisualizationIndexService:
    ...
    def _count_links_by_year(self, annual_report_dir: Path, years: list[int]) -> dict[str, dict[str, Any]]:
        ...
        for index, year in enumerate(years, start=1):
            filtered_out_path = annual_report_dir / str(year) / FILTERED_OUT_FILENAME
            count = self._count_jsonl_rows(filtered_out_path)
            ...
            result[str(year)] = {
                "year": year,
                "count": count,
                "status": "completed" if filtered_out_path.exists() else "pending",
                "active": False,
                "updatedAt": _now_text(),
            }
        ...
        return {
            "_snapshot": {
                ...
                "countBasis": f"counts come from {FILTERED_OUT_FILENAME}",
